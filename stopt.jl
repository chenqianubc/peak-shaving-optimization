...
w1 = 0 #w1 is the weight of two objectives, which can be adjusted
...
#----------------------------Objective ---------------------- -----------------
@NLobjective(gp, Min, w1*sum((sum(4*100*hed[1,j,k]*9.8*(M[1,j,k]/4)/1000/xiaolv[1,j,k] for j in 1:nt-1)
...
e2*sum(UGS[j,k]*30*60/(0.0417*16.3776) for j in 1:nt-1) for k in 1:ns)/ns - (1-w1)*sum(sum((P[i,nt,k]+P[i+1,nt,k])/2 for i in 1:16)*(10^7)*A/a/a +
sum((P[i,nt,k]+P[i+1,nt,k])/2 for i in 18:33)*(10^7)*A/a/a + sum((P[i,nt,k]+P[i+1,nt,k])/2 for i in 35:50)*(10^7)*A/a/a +
sum((P[i,nt,k]+P[i+1,nt,k])/2 for i in 52:67)*(10^7)*A/a/a + sum((P[i,nt,k]+P[i+1,nt,k])/2 for i in 69:94)*(10^7)*A/a/a for k in 1:ns)/ns/(0.0417*16.3776)/10)
#-------------------------- Initial values------------------------------------
...
@NLconstraint(gp, [j = 2:nt-1, k = 1:ns], abs(UGS[j,k] - UGS[j-1,k]) <= 5)

optimize!(gp)

println("gas withdrawn cost:")
println(round(sum(e2*sum(value(UGS[j,k])*30*60/(0.0417*16.3776) for j in 1:nt-1) for k in 1:ns)/ns))
